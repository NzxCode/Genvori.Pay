// src/Components/VerifyResetOTP.tsx
...
interface VerifyResetOTPProps {
  email: string;
  onNavigateBack: () => void;
  onVerifySuccess: (accessToken: string) => void;
}

const OTP_LENGTH = 6;
const RESEND_SECONDS = 60;

export default function VerifyResetOTP({ email, onNavigateBack, onVerifySuccess }: VerifyResetOTPProps) {
  ...
  const handleResend = () => {
    if (countdown > 0) return;
    console.log("Resend Reset OTP ke:", email);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.heroSection}>
            ...
            <Text style={styles.brandName}>Verifikasi <Text style={styles.brandAccent}>Kode</Text></Text>
            <Text style={styles.brandTagline}>Masukkan kode untuk melanjutkan reset password</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  emailText: { fontSize: 13, color: '#4338CA', marginLeft: 8, textAlign: 'center' },
  ...
  countdownText: { color: '#1E293B', fontWeight: 'bold' },
